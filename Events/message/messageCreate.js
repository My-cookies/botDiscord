...
module.exports = async (bot, message) => {
    ...
    if (command.help.permissions) {
        if (!await message.channel.permissionsFor(message.author).has(command.help.permissions)){
            return message.channel.send(`Il vous faut la permission : \`${command.help.permissions}\``)
        }
    }
